...
> Have a BGP-capable router (e.g. a UniFi Cloud Gateway) and want active-active anycast instead of a floating VIP? See [README-anycast.md](README-anycast.md). Back to the [index](README.md).
...
2.  **Synchronize Pi-hole Configurations (Crucial!):**
    ...
    * Use [`install-sync.sh`](install-sync.sh) (see [README-sync.md](README-sync.md)) for a ready-made Teleporter-API-based sync between nodes.
...
3.  **Test Failover:**
    ...
    * Restore the original `MASTER` node and observe if it reclaims the VIP (by default, `keepalived` preempts, so the higher-priority node always reclaims MASTER once healthy).
